...
class PicturePuzzleRepository {
  ...
  static getPicturePuzzleDataList(int level) {
    ...
    while (list.length < 5) {
      ...
      picturePuzzleDataList.asMap().forEach((
        int i,
        PicturePuzzleData picturePuzzleData,
      ) {
        puzzleList.add(
          PicturePuzzleShapeList([
            PicturePuzzleShape(
            ...
            PicturePuzzleShape(
              text:
                  i == picturePuzzleDataList.length - 1
                      ? "?"
      ...
      list.add(
        PicturePuzzle(puzzleList, int.parse(picturePuzzleDataList.last.text)),
      );
    }
    return list;
  }
    ...
    if (level == 1) {
      switch (index) {
        ...
        case 1:
          ...
          list.add(
            getRowFirst(listShape[0], listSign[0], listSign[1], listDigit[0]),
          );
          list.add(
            getRowSecond(
              listShape[0],
        ...
        case 4:
          ...
          while (listDigit.length < 3) {
            MathUtil.generateRandomNumber(level, 10 + level, 3).forEach((
              digit,
            ) {
              if (!listDigit.contains(digit)) {
                listDigit.add(digit);
