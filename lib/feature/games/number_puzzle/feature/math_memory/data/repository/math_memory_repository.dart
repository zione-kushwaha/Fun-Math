  ...
  List<MathPair> _generateEasyPairs(int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (_random.nextBool()) {
        ...
        final maxVal = math.max(a, b);
        final minVal = math.min(a, b);
        pairs.add(MathPair(
    ...
    for (int i = 0; i < count; i++) {
      ...
      switch (operation) {
        case 0: // Addition
          ...
          break;        case 1: // Subtraction     
               final maxVal = _random.nextInt(20) + 10; // 10-30
          final minVal = _random.nextInt(math.min(maxVal, 10)) + 1; // 1-10 or 1-maxVal
          pairs.add(MathPair(
            question: "$maxVal - $minVal = ?",
            answer: "${maxVal - minVal}",
          ));
    ...
    for (int i = 0; i < count; i++) {
      ...
      switch (operation) {
        ...
        case 1: // Subtraction with larger numbers
          final max = _random.nextInt(90) + 10; // 10-100
          final min = _random.nextInt(max - 5) + 5; // 5-(max-5)
          pairs.add(MathPair(
            question: "$max - $min = ?",
            answer: "${max - min}",
          ));
